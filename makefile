...
# Znacznik czasowy: 2024-01-29 20:49:53.541904
#

OUTDIR		:= build-dir

CC          := gcc
CC_FLAGS    := -fmax-errors=5 -xc -D_ANSI_OUTPUT -fdiagnostics-color -Werror=vla -Wno-error=unused-parameter -DINSIDE_DANTE -Wall -ggdb3 -Wno-error=implicit-fallthrough -Wno-parentheses -Wno-error=parentheses -Werror -pedantic -D_GNU_SOURCE -std=c11 -Wextra 
...
LD          := gcc
LD_FLAGS    := -Wl,-cref -Wl,-wrap,main -ggdb3 -Wl,-Map=main.map 
LD_LIBS     := -lm 
